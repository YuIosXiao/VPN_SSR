...
import com.wwws.wwwsvpn.myapplication.utils.DateUtils;
...
public class CalendarView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 获取当月第一天位于周几
        int firstWeekDay  = DateUtils.getFirstDayWeek(year, month);
        ...
        int lastMonthDays  = DateUtils.getMonthDays(lastYear, lastMonth);
        // 绘制每天
        for (int day = 0; day < 42; day++) {
            int column = day%7;
            int row = (day - day%7)/7;
            if (day < firstWeekDay){
                String dayStr = String.valueOf(lastMonthDays - firstWeekDay + column + 1);
                ...
                drawText(canvas, dayStr, getResources().getColor(R.color.lightblack), mTextSize, x, y);
            } else if (day > (firstWeekDay + monthDays - 1)) {
                String dayStr = String.valueOf(day - firstWeekDay - monthDays + 1);
                float textWidth = mPaint.measureText(dayStr);
                int x = (int) (mColumnWidth * column + (mColumnWidth - textWidth) / 2);
                int y = (int) (mRowHeight * row + mRowHeight / 2 - (mPaint.ascent() + mPaint.descent()) / 2);
                drawBackground(canvas, mDayBackground, column, row);
                ...
                String dayStr = String.valueOf(day - firstWeekDay + 1);
                float textWidth = mPaint.measureText(dayStr);
                int x = (int) (mColumnWidth * column + (mColumnWidth - textWidth) / 2);
                int y = (int) (mRowHeight * row + mRowHeight / 2 - (mPaint.ascent() + mPaint.descent()) / 2);
                if(mSelectDate == null || mSelectDate.size() == 0 || !mSelectDate.contains(getFormatDate(year, month - 1, day - firstWeekDay + 1))){
                    // 没有则绘制默认背景和文字颜色
                    drawBackground(canvas, mDayBackground, column, row);
